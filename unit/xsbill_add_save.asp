...
'response.write q
		  ...
		  if queryrs.eof then
	 	  	...
	 	  	'response.write "tNum"&queryrs("tNum")
	 	  	 dim TempStockNumber
...
if qBillCode = "" then
	...
	on error resume next
		...
		for i=1 to intK
			...
			rsdetail.close
			InPrice = trim(request("inprice_"&i))
	...
	if err.number=0 then
		tempmsg="新增单据，单号：" &BillCode
		manageLog=tempmsg
		conn.CommitTrans
		conn.close
		set conn=nothing
		Response.Write"<script>alert('"&tempmsg&"');self.opener.location.href='xsbill_add.asp?billtype="&billtype&"&refresh=add';window.opener=null;window.close();</SCRIPT>" 
	else
		conn.RollbackTrans
...
if qBillCode <> "" then
  ...
  on error resume next
		...
		for i=1 to intK
			...
			rsdetail.close
			'InPrice = trim(request("inprice_"&i))
      InPrice=trim(request("Money_"&i))
			'response.write "number"&i &":"&trim(request("number_"&i))
			InMoney = InPrice*trim(request("number_"&i))
			q = ""
			q = q&"insert into tBillDetail("
			q = q&"DBillCode, GoodsStyle, GoodsName,"
		...
		set rsdetail=nothing
	if err.number=0 then
		tempmsg="修改单据，单号：" &qBillCode
		manageLog tempmsg
		conn.CommitTrans
		conn.close
		set conn=nothing
		Response.Write"<script>alert('"&tempmsg&"');self.opener.location.href='xsbill_add.asp?billcode="&qBillCode&"&refresh=edit';window.opener=null;window.close();</SCRIPT>" 
	else
		conn.RollbackTrans
		Response.Write err.description
	end if
