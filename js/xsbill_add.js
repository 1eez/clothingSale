function validate_form(){
	...
	for(j=1;j<parseInt(document.all.intK.value)+1;j++){
		 var tempnum=document.all('number_'+j).value;
		 var tempobj=document.all('number_'+j)
		 if(tempnum=="" || tempnum=="0"){
		    	validity = false; alert('请您录入商品数量！');tempobj.focus(); 
		 }
...
function setbarcode(){
	if(event.keyCode==13){
			openwin('../unit/barcodein.asp?number='+document.all.number.value+'&barcode='+document.all.barcode.value,120,80)
		document.all.barcode.select();
	}
}
...
function checkdepot(el){
	...
	{
		openwin('../unit/stock_select.asp?type=ck&id='+el+'&DepotName='+document.all.DepotName.value,800,600)
	}
}
function checkdepothp(el){
	var k
	k=window.event.srcElement.parentElement.parentElement.rowIndex;
	if(document.all.DepotName.value=='请选择'){alert('请选择仓库')}
	if(document.all.DepotName.value!='请选择')
	{
		openwin('../unit/stock_select_hp.asp?type=ck&id='+el+'&DepotName='+document.all.DepotName.value,800,600)
	}
}
function addNew(){
	...
	tr.insertCell().innerHTML='<input onkeyup=changemoney('+k+') style=width:100%;text-align:right value=0 type=text name=cmoney'+k+'>'
...
function del(){
	...
	for(k=0;k<document.all.t136.rows.length-1;k++){
		...
		document.all.t136.rows[k+1].cells[11].children[1].name="inprice_"+(k+1);
	}
	document.all.intK.value=k
}
