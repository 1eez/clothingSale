function setbarcode(){
	if(event.keyCode==13){
		...
		if(document.all.depot.value!='请选择'){
			addNew();
			openwin('../public/ckbarcode_select.asp?depot='+document.all.depot.value+'&id='+document.all.intK.value+'&barcode='+document.all.barcode.value,640,480)
		}
	}
}

function openrk(){
	if(document.form.cktype.value.indexOf('supply')>-1){
		openwin("../public/cust_select.asp?co_type=supply",640,480)
	}
	if(document.form.cktype.value.indexOf('depart')>-1){
		openwin("../public/mtreehp.asp?table=de1part",340,300)
	}
	if(document.form.cktype.value.indexOf('custom')>-1){
		openwin("../public/cust_select.asp?co_type=custom",640,480)
	}
	if(document.form.cktype.value.indexOf('other')>-1){
	}
}
...
function changecaption(){
	if(document.form.cktype.value.indexOf('supply')>-1){
		...
		document.all.ckmc.innerHTML='退货仓：';
	...
	if(document.form.cktype.value.indexOf('depart')>-1){
		...
		document.all.ckmc.innerHTML='出库仓：';
	...
	if(document.form.cktype.value.indexOf('custom')>-1){
		...
		document.all.showcustname.style.display="";
		document.all.typecode.value = "";
		document.all.ckmc.innerHTML='出货仓：';
...
validity = true;
if (!check_empty(document.form.ckdate.value))
{ validity = false; alert('出库日期不能为空，请您重新选择！'); }

if (!check_empty(document.form.cktype.value))
{ validity = false; alert('请您选择出库类型！'); }
...
function checkdepot(el){
	if(document.all.depot.value=='请选择'){alert('请选择仓库')}
	if(document.all.depot.value!='请选择')
	{openwin('../public/hpzl_select.asp?type=ck&id='+el+'&depot='+document.all.depot.value,640,480)}
}
function addNew(){
