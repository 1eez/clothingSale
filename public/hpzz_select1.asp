...
q = "select * from t_goods where 1=1 "&sqlwhere&" and 1=1 "&"order by "&orderf&" "&ordero
sqlexcel = q
...
function getindex(){
	...
	self.opener.document.all('goodscode').value = 
			tab.rows[obj.rowIndex].cells[1].innerHTML
	self.opener.document.all('goodsname').value = 
			(tab.rows[obj.rowIndex].cells[2].innerHTML).replace(/<\/?.+?>/g,'')
	self.opener.document.all('goodsunit').value = 
			tab.rows[obj.rowIndex].cells[3].innerHTML
	self.opener.document.all('units').value = 
			tab.rows[obj.rowIndex].cells[4].innerHTML
	self.opener.document.all('number').value = 1
...
<div style="width:100%;height:40px;line-height:25px;padding:2px 0px;">
	<form name=form action="hpzz_select1.asp?id=<%=request.querystring("id")%>" method="get">
